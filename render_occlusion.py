...
import numpy as np
import warnings
from gaussian_renderer import render_occ, render_occ_wrapper
...
def render_sets(iteration, scene : Scene, renderFunc, renderArgs, env_map=None):
    ...
    for config in validation_configs:
        if config['cameras'] and len(config['cameras']) > 0:
            ...
            occlusion_path = os.path.join(args.model_path, "render_result", "occlusion")
            sun_shadow = os.path.join(args.model_path, "render_result", "sun_shadow")
            os.makedirs(occlusion_path, exist_ok=True)
            os.makedirs(sun_shadow, exist_ok=True)
            ...
            print("sun direction:", sun_dir.tolist())
            ...
            for camera_id, viewpoint in enumerate(tqdm(config['cameras'], bar_format="{l_bar}{bar:50}{r_bar}")):
                ...
                #_, _, render_pkg2 = renderFunc(args, viewpoint, gaussians, background, scene.time_interval, env_map, iteration, camera_id,mask=mask)
                ...
                occlusion_img.save(os.path.join(occlusion_path, f"{viewpoint.colmap_id:03d}_occ.png"))
                #################
                static_occlusion = render_pkg["static_occlusion"].permute(1, 2, 0)
                static_occlusion = static_occlusion.permute(2, 0, 1)
                static_occlusion = static_occlusion.clamp(0.0, 1.0)
                to_pil = transforms.ToPILImage()
                static_occlusion_img = to_pil(static_occlusion)
                static_occlusion_img.save(os.path.join(occlusion_path, f"{viewpoint.colmap_id:03d}_static_occ.png"))

                #########################
                sun_shadow_map = render_pkg["sun_shadow_map"].permute(1, 2, 0)
                # occlusion = render_pkg['occlusion_map']
                sun_shadow_map = sun_shadow_map.permute(2, 0, 1)
                #occlusion = (occlusion * alpha_mask + background[:, None, None] * (1.0 - alpha_mask)).clamp(0.0, 1.0)
                sun_shadow_map = sun_shadow_map.clamp(0.0, 1.0)
                to_pil = transforms.ToPILImage()
                sun_shadow_map_img = to_pil(sun_shadow_map)

                sun_shadow_map_img.save(os.path.join(sun_shadow, f"{viewpoint.colmap_id:03d}_sun_shadow.png"))


                static_sun_shadow_map = render_pkg["static_sun_shadow"].permute(1, 2, 0)
                # occlusion = render_pkg['occlusion_map']
                static_sun_shadow_map = static_sun_shadow_map.permute(2, 0, 1)
                #occlusion = (occlusion * alpha_mask + background[:, None, None] * (1.0 - alpha_mask)).clamp(0.0, 1.0)
                static_sun_shadow_map = static_sun_shadow_map.clamp(0.0, 1.0)
                to_pil = transforms.ToPILImage()
                static_sun_shadow_map_img = to_pil(static_sun_shadow_map)

                static_sun_shadow_map_img.save(os.path.join(sun_shadow, f"{viewpoint.colmap_id:03d}_static_sun_shadow.png"))
...
if __name__ == "__main__":
   ...
   # render_sets(first_iter, scene, render_wrapper, (args, background), env_map=env_map)
    render_sets(first_iter, scene, render_occ_wrapper, (args, background), env_map=env_map)
