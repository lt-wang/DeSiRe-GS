    ...
    for config in validation_configs:
        if config['cameras'] and len(config['cameras']) > 0:
            ...
            for camera_id, viewpoint in enumerate(tqdm(config['cameras'], bar_format="{l_bar}{bar:50}{r_bar}")):
                ...
                # booeal_dynamick_
                #depth_normal = render_pkg.get("depth_normal", gt_normal)
                ...
                if dynamic_mask.sum() > 0:
                    dynamic_mask = dynamic_mask.float().repeat(3, 1, 1) > 0 # (C, H, W)
                    #save_image(dynamic_mask, os.path.join(dynamic_path, f"{viewpoint.colmap_id:03d}_dynamic_mask.png"))
