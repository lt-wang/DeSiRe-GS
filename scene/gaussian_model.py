...
class GaussianModel:
    ...
    def save_ply_at_t(self, path, time=0.0):
        ...
        rotation = self._rotation.detach().cpu().numpy()
        scale_upper_bound = np.exp(scale).max(axis=1)
        print("Before filtering, number of scale:{}, range:{:.4f}~{:.4f}".format(scale_upper_bound.shape[0], scale_upper_bound.min(),  scale_upper_bound.max()) )

        mask = (marginal_t[:, 0].detach().cpu().numpy() > 0.05) & (self.get_opacity[:, 0].detach().cpu().numpy() > 0.05)
